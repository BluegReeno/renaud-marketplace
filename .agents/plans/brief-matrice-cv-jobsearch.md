# BRIEF — Matrice Job Search × Skill `cv-generator`

> **Destinataire** : Claude Code, pour upgrader le skill `cv-generator` (plugin anthropic-skills).
> **Objectif** : passer de 3 modes génériques (`ai_consulting`, `cto`, `business_dev`) à une matrice **5 profils × 5 types d'entreprise**, remplie uniquement sur les **15 cases crédibles**. Pour chaque case : texte de crédibilité, titre CV, 3 lignes about, 3 titres de containers sur-mesure + contenu — en **FR et EN**.
> **Statut** : étape 1 du process (recherche + classification + rédaction). À valider par Renaud avant remise à Claude Code.
...
- Titre EN : `CTO — Scaling AI-Powered Solutions for Climate & Industry` (≈ mode `cto` actuel, conservé)
...
# 6. Implémentation skill `cv-generator` — spécification pour Claude Code

## 6.1 Changements de structure
...
4. **Fiches descriptives par profil** : créer `cv-generator/profiles/{p1_architecte,p2_lead,p3_cto,p4_cs_fde,p5_sales}.md` — définition, cases couvertes, règles narratives (dont neutralisation solopreneur), exemples de candidatures réelles.
5. **Bullets d'expérience** : décliner les bullets `blue_green`/`artelia`/`open_ocean` par profil (5 jeux au lieu de 3). P4 et P5 réutilisent la base `business_dev` avec les ajustements de narratif ci-dessus (ordre des expériences selon la règle solopreneur).
...
- Template SVG inline (weasyprint ne rend pas les emoji) ; vérification 1 page A4 via pikepdf ; couleurs containers fixes (gold/vert/bleu) ; template de référence `jobsearch/cv_template_fixed.html`.

## 6.3 Validation

- Générer les 15 CV FR + 15 EN en batch test, vérifier 1 page partout.
- Test prioritaire sur le pipeline actif : Relief (P3×T1), Dust (P4×T5), ILLUIN AE (P5×T3), ILLUIN Lead Architect (P1×T3), Oplit (P4×T1).

---

# 7. Prochaines étapes (process global)

1. ✅ Étape 1 — ce brief (recherche + classification + rédaction). **LIVRÉ.**
2. ⏳ Étape 2 — Renaud valide / améliore (textes de crédibilité, titres de containers, choix de cases).
3. ⏳ Étape 3 — remise à Claude Code : upgrade du skill + fiches par profil (section 6).
