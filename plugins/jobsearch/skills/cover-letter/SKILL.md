...
allowed-tools: "Read mcp__plugin_briefing_gmail-mcp__draft_email Bash(python3 *)"
---
...
## Step 6 — Optional: create a Gmail draft (via gmail-mcp)

After outputting the letter, ask once:

> "Voulez-vous que je crée un brouillon Gmail avec cette lettre ? (email du recruteur requis — vous pouvez aussi joindre un CV PDF)"

If the user says **no** or does not reply, stop here.

If the user says **yes**, collect:
1. **Recruiter email** — required; reject if missing.
2. **CV path** — optional. Absolute path to a PDF previously generated by `/cv-generator` (e.g. `~/Library/CloudStorage/SynologyDrive-MyAssistant/vault/jobsearch/CV_Poste_Entreprise.pdf`). If provided, the PDF is attached.

### Building the draft

**Subject line:**
- French letter → `"Candidature — <Poste> chez <Entreprise>"`
- English letter → `"Application — <Role> at <Company>"`

**Body:** the cover letter text verbatim (plain text).

**Attachment (if CV path provided):**

Read the PDF as base64 using stdlib Python (no packages):

```bash
python3 -c "import base64,sys; print(base64.b64encode(open(sys.argv[1],'rb').read()).decode())" "<CV_PATH>"
```
...
**Invoke `mcp__plugin_briefing_gmail-mcp__draft_email`:**
  ...
  "attachments": [
    {
      ...
      "content": "<base64 string — no newlines>"
    }
  ]
}
```

Omit `attachments` entirely if no CV path was provided.

**On success:** report `✅ Brouillon Gmail créé — ouvrez Gmail pour relire et envoyer.`

**On error:** report the error verbatim. Do not retry automatically.
